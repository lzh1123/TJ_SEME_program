    ...
    def create(self, topic: str, theme: Optional[str] = None, use_rag: bool = True) -> PresentationBundle:
        ...
        if use_rag and self._rag:
            ...
            try:
                ...
                # Log retrieval for RAG evaluation
    ...
    def generate_outline(self, topic: str, theme: Optional[str] = None, use_rag: bool = True) -> dict:
        ...
        if rag_enabled:
            ...
            try:
                ...
                # Log retrieval for RAG evaluation (use topic as tracking id for outlines)
                if log_retrieval is not None and rag_context:
                    try:
                        result = self._rag.search(topic, top_k=8)
    ...
    def regenerate(self, presentation_id: str, topic: Optional[str] = None, section: Optional[str] = None, use_rag: bool = True) -> PresentationBundle:
        ...
        if use_rag and self._rag:
            ...
            try:
                ...
                logger.info("RAG retrieve_context for regenerate topic=%r: %d chars in %.1fs", base_topic[:80], len(rag_context), time.time() - t0)
            except Exception as e:
                logger.warning("RAG retrieve_context FAILED for regenerate topic=%r: %s: %s", base_topic[:80], type(e).__name__, e)
                rag_context = ""
